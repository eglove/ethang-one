...
export class UtilNrwl {
  ...
  projectsWithDependencies(projectNames: string[]): string[] {
    const filteredProjects: string[] = projectNames.filter(projectName => {
      return Boolean(projectName) && this.isProject(projectName);
    });
      ...
      for (const projectName of projectNames) {
        projectNamesWithDependencies = [
          ...
          ...this.projectsWithDependencies(
            this.dependencies[projectName].map(dependency => {
    ...
    return projectNamesWithDependencies.filter((item, index, array) => {
  ...
  writeAvaOneDependencies(): void {
    ...
    if (avaOneDependencies) {
      dependencyRoots = this.getExcludedProjectRoots([
        'ava-one',
        ...avaOneDependencies.map(dependency => {
          return dependency.target;
        }),
      ]);
    }
    ...
    dependencyRoots = [
      '/*',
      ...dependencyRoots.map(dependencyRoot => {
        return `!${dependencyRoot}/src`;
      }),
    ];
  ...
  getExcludedProjectRoots(projectNames: string[]): string[] {
    ...
    if (projectNames.length > 0) {
      filteredProjects = this.projectNodes?.filter(project => {
        return (
    ...
    if (filteredProjects) {
      excludedProjects = filteredProjects.map(project => {
        return project.name;
      });
    }
  ...
  findApp(projectName: string): ProjectGraphNode | undefined {
    return this.projectNodes?.find(project => {
      return project.name === projectName && this.isApp(project);
    });
  }

  findAppLike(projectName: RegExp): ProjectGraphNode | undefined {
    return this.projectNodes?.find(project => {
      return projectName.test(project.name) && this.isApp(project);
    });
  }
