...
export const HermitStreamLink = ({hermitStream}: HermitStreamLinkProperties): JSX.Element => {
  const hermitDispatch = useHermitDispatchContext();

  const handleSetCurrentStream = (): void => {
    hermitDispatch({
      payload: hermitStream.id,
      type: HermitActionEnum.setHermitWatching,
    });
  };
  ...
  if (hermitStream) {
    ...
    return (
      <button className={styles.StreamLink as string} type="button" onClick={handleSetCurrentStream}>{`${user_name} - ${title}`}</button>
    );
  }
